...
const { onDifficulty, getInfo, onRound, doesGameExist, onUserUpdate, updateUsers, startGame, onGrid, onStatus, getStatus, updateRound, updateDifficulty } =
  useSocket();
...
<template>
  <div
    v-if="showPage"
    class="bg-gradient-to-r from-red-700 to-red-800 h-screen flex justify-center items-center"
  >
    ...
    <div class="flex flex-col space-y-2 text-white" v-if="gameStatus == 'MENU'">
      ...
      <select v-model="roundMax" class="text-black disabled:opacity-75 disabled:cursor-not-allowed" @change="handleChangeRound" :disabled="!player?.host">
        <option v-for="round in roundsList" :value="round" class="text-black">{{ round }}</option>
      </select>

      <select v-model="difficulty" class="text-black disabled:opacity-75 disabled:cursor-not-allowed" @change="handleChangeDifficulty" :disabled="!player?.host">
        <option v-for="diff in difficulties" :value="diff" class="text-black">{{ diff }}</option>
      </select>
    </div>
  </div>
</template>
